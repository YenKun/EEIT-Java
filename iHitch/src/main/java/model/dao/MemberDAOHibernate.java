...
public class MemberDAOHibernate implements MemberDAO {
	...
	public Member select(String email) {
		return this.getSession().createQuery("FROM Member WHERE email= :email",Member.class).setParameter("email", email).uniqueResult();
	...
	public Integer insert(Member bean) {
		if (bean != null) {
			Member temp = this.getSession().createQuery("FROM Member WHERE email= :email",Member.class).setParameter("email", bean.getEmail()).uniqueResult();
			if (temp == null) {
